...
class AmsSlot:
    ...
    #: Какое сопло кормит это место заправки, в нумерации команды печати
    #: (`0` — правое, `1` — левое). `None` — сказать нечего: односопловая
    #: машина, подающая система, привязанная к обоим соплам переключателем, либо
    #: прошивка, которая привязки не сообщает.
    #:
    #: Нужно машинам с двумя соплами: плита нарезана под конкретное сопло, и
    #: филамент обязан прийти из места, которое это сопло кормит. Без этого хаб
    #: подбирал слот по одному лишь типу пластика и отдавал принтеру заведомо
    #: невыполнимую раскладку — тот отвечал `0700-8012` и вставал на паузу.
